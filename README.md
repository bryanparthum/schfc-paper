...
Estimation of the three models and their SC-HFCs (MimiGIVE and MimiIWG as well as a hybrid of the two combining FAIRv1.6.2 with MimiIWG) is outlined below. For convenience, this repository already includes the completed model runs (in each models `output` subdirectory), and the full distributions of their estimates (in each model's `output\scghgs` subdirectory).
